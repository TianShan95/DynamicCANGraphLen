...
class GraphSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, graph_origin, graphs_coarsen, num_pool_matrix, num_pool_final_matrix, features='default', normalize=True,
        ...
        # if max_num_nodes == 0:
        #     self.max_num_nodes = max(graph_origin.number_of_nodes())
        # else:
        ...
        adj = np.array(nx.to_numpy_matrix(graph_origin))
        if normalize:  # adg 正则化
            sqrt_deg = np.diag(1.0 / np.sqrt(np.sum(adj, axis=0, dtype=float).squeeze()))
            adj = np.matmul(np.matmul(sqrt_deg, adj), sqrt_deg)  # 得到 重归一化 形式的 拉普拉斯矩阵
        self.adj_all.append(adj)
        self.len_all.append(graph_origin.number_of_nodes())  # 节点数量
        self.label_all.append(graph_origin.graph['label'])  # 图标签
        if features == 'default':
            f = np.zeros((self.max_num_nodes, self.feat_dim), dtype=float)
            for i, u in enumerate(graph_origin.nodes()):  # 遍历图中所有的节点 但是特征矩阵初始化是以最大的节点大小初始化的 所以结尾的行存在全零行
                f[i, :] = graph_origin.nodes[u]['feat']
            self.feature_all.append(copy.deepcopy(f))
        elif features == 'id':
            self.feature_all.append(np.identity(self.max_num_nodes))
        elif features == 'deg-num':
            degs = np.sum(np.array(adj), 1)
            degs = np.expand_dims(np.pad(degs, [0, self.max_num_nodes - graph_origin.number_of_nodes()], 0),
                                  axis=1)
            ...
            degs = np.pad(feat, ((0, self.max_num_nodes - graph_origin.number_of_nodes()), (0, 0)),
                          'constant', constant_values=0)

            clusterings = np.array(list(nx.clustering(graph_origin).values()))
            clusterings = np.expand_dims(np.pad(clusterings,
                                                [0, self.max_num_nodes - graph_origin.number_of_nodes()],
                                                'constant'),
                                         axis=1)
            g_feat = np.hstack([degs, clusterings])
            if 'feat' in graph_origin.node[0]:
                node_feats = np.array([graph_origin.node[i]['feat'] for i in range(graph_origin.number_of_nodes())])
                node_feats = np.pad(node_feats, ((0, self.max_num_nodes - graph_origin.number_of_nodes()), (0, 0)),
                                    'constant')
...
class GraphSampler1(torch.utils.data.Dataset):
    ''' Sample graphs and nodes in graph
    '''
