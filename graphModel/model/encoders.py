...
class GcnEncoderGraph(nn.Module):
    def __init__(self, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        '''
        :param input_dim: 图特征的维度 本次使用的节点的标号  维度为 1
        :param hidden_dim: args.hidden_dim
        :param embedding_dim: args.output_dim 图网络 最后输出的 最后 一个节点的 向量维度
        :param label_dim:  args.num_classes
        :param num_layers:
...
class WavePoolingGcnEncoder(GcnEncoderGraph):
    ...
    def forward(self, x, adj, adj_pooled_list, batch_num_nodes, batch_num_nodes_list, pool_matrices_dic, **kwargs):
        ...
        # print(out.shape)
        out_all.append(out)
        if self.num_aggs == 2:  # self.num_aggs = 1
